...
import { DreReport } from '../types/finance';
import {
  ChevronDown,
  ChevronRight,
  Minus,
    ...
    <div className="glass-panel rounded-2xl p-4 sm:p-6 lg:p-7">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 sm:pb-5 border-b border-slate-800 gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="px-3 py-1.5 rounded-lg bg-slate-800/80 border border-slate-700/60 text-xs font-medium text-slate-300">
            Margem Bruta:{' '}
          ...
          <div className="px-3 py-1.5 rounded-lg bg-slate-800/80 border border-slate-700/60 text-xs font-medium text-slate-300">
            Margem Op.:{' '}
            <span className="font-mono-numbers font-semibold text-blue-400">
      ...
      {/* DRE Rows */}
      <div className="mt-4 sm:mt-5 space-y-2.5 font-mono-numbers">
        {/* 1. (+) RECEITAS TOTAIS */}
        <div className="rounded-xl bg-slate-900/60 border border-emerald-500/20 p-3.5 transition">
          <div
            onClick={() => toggleSection('incomes')}
            className="flex items-center justify-between cursor-pointer select-none gap-2"
          >
            <div className="flex items-center space-x-3 font-sans min-w-0 flex-1">
              <span className="p-1 rounded-md bg-emerald-500/10 text-emerald-400 shrink-0">
                <Plus className="h-4 w-4" />
              </span>
              <div className="min-w-0">
                <span className="text-xs sm:text-sm font-semibold text-emerald-400 uppercase tracking-wider block sm:inline">
                  (+) Receitas Totais
                </span>
                <span className="text-[11px] sm:text-xs text-slate-400 font-normal sm:ml-2 block sm:inline truncate">
          ...
          {expandedSections.incomes && dre.breakdown.incomes.length > 0 && (
            <div className="mt-3 pt-2.5 border-t border-slate-800 space-y-1.5 pl-3 sm:pl-8 font-sans text-xs">
              {dre.breakdown.incomes.map((item) => (
                <div
                  key={item.id}
                  className="flex items-center justify-between text-slate-300 py-0.5 hover:text-white gap-2"
                >
                  <span className="truncate text-[11px] sm:text-xs min-w-0 flex-1">{item.name}</span>
                  <div className="flex items-center space-x-3 sm:space-x-4 font-mono-numbers shrink-0">
        ...
        {/* 2. (-) CUSTOS FIXOS */}
        <div className="rounded-xl bg-slate-900/60 border border-rose-500/20 p-3.5 transition">
          <div
            onClick={() => toggleSection('fixedCosts')}
            className="flex items-center justify-between cursor-pointer select-none gap-2"
          >
            <div className="flex items-center space-x-3 font-sans min-w-0 flex-1">
              <span className="p-1 rounded-md bg-rose-500/10 text-rose-400 shrink-0">
                <Minus className="h-4 w-4" />
              </span>
              <div className="min-w-0">
                <span className="text-xs sm:text-sm font-semibold text-rose-400 uppercase tracking-wider block sm:inline">
                  (-) Custos Fixos
                </span>
                <span className="text-[11px] sm:text-xs text-slate-400 font-normal sm:ml-2 block sm:inline truncate">
        ...
        <div className="rounded-xl bg-slate-900 border border-slate-700/80 p-3.5 flex items-center justify-between shadow-sm gap-2">
          <div className="flex items-center space-x-3 font-sans min-w-0 flex-1">
            <span className="p-1 rounded-md bg-slate-800 text-slate-300 shrink-0">
              <Equal className="h-4 w-4" />
            </span>
            <div className="min-w-0">
              <span className="text-xs sm:text-sm font-semibold text-white uppercase tracking-wider block sm:inline">
                (=) Res. Operacional Bruto
              </span>
              <span className="text-[11px] sm:text-xs text-slate-400 sm:ml-2 block sm:inline truncate">
          ...
          <div className="flex items-center space-x-3 sm:space-x-4 shrink-0">
            <span
              className={`text-xs sm:text-base font-bold text-right ${
                dre.grossOperatingResult >= 0 ? 'text-blue-400' : 'text-rose-400'
              }`}
            >
              {formatBRL(dre.grossOperatingResult)}
            </span>
            <span className="text-blue-300 font-semibold text-[10px] sm:text-xs w-9 sm:w-12 text-right shrink-0">
              {dre.margins.grossMargin}%
            </span>
            <div className="w-4 shrink-0" />
          </div>
        ...
        {/* 4. (-) CUSTOS VARIÁVEIS */}
        <div className="rounded-xl bg-slate-900/60 border border-amber-500/20 p-3.5 transition">
          <div
            onClick={() => toggleSection('variableCosts')}
            className="flex items-center justify-between cursor-pointer select-none gap-2"
          >
            <div className="flex items-center space-x-3 font-sans min-w-0 flex-1">
              <span className="p-1 rounded-md bg-amber-500/10 text-amber-400 shrink-0">
                <Minus className="h-4 w-4" />
              </span>
              <div className="min-w-0">
                <span className="text-xs sm:text-sm font-semibold text-amber-400 uppercase tracking-wider block sm:inline">
                  (-) Custos Variáveis
                </span>
                <span className="text-[11px] sm:text-xs text-slate-400 font-normal sm:ml-2 block sm:inline truncate">
        ...
        <div className="rounded-xl bg-slate-900 border border-slate-700/80 p-3.5 flex items-center justify-between shadow-sm gap-2">
          <div className="flex items-center space-x-3 font-sans min-w-0 flex-1">
            <span className="p-1 rounded-md bg-slate-800 text-emerald-400 shrink-0">
              <Equal className="h-4 w-4" />
            </span>
            <div className="min-w-0">
              <span className="text-xs sm:text-sm font-semibold text-white uppercase tracking-wider block sm:inline">
          ...
          <div className="flex items-center space-x-3 sm:space-x-4 shrink-0">
            ...
            <span className="text-emerald-400 font-semibold text-[10px] sm:text-xs w-9 sm:w-12 text-right shrink-0">
              {dre.margins.operatingSavingsMargin}%
            </span>
            <div className="w-4 shrink-0" />
          </div>
        ...
        {/* 6. (-) INVESTIMENTOS */}
        <div className="rounded-xl bg-slate-900/60 border border-indigo-500/20 p-3.5 transition">
          <div
            onClick={() => toggleSection('investments')}
            className="flex items-center justify-between cursor-pointer select-none gap-2"
          >
            <div className="flex items-center space-x-3 font-sans min-w-0 flex-1">
              <span className="p-1 rounded-md bg-indigo-500/10 text-indigo-400 shrink-0">
                <Minus className="h-4 w-4" />
              </span>
              <div className="min-w-0">
                <span className="text-xs sm:text-sm font-semibold text-indigo-400 uppercase tracking-wider block sm:inline">
        ...
        <div className="rounded-xl bg-slate-900 border-2 border-emerald-500/40 p-4 sm:p-5 flex items-center justify-between shadow-sm gap-2">
          <div className="flex items-center space-x-3 font-sans min-w-0 flex-1">
            <span className="p-1.5 rounded-lg bg-emerald-500/10 text-emerald-400 shrink-0">
              <Equal className="h-4 w-4 sm:h-5 sm:w-5" />
            </span>
            <div className="min-w-0">
              <span className="text-xs sm:text-base font-bold text-white uppercase tracking-wider block sm:inline">
                (=) Fluxo Líquido Final
              </span>
              <span className="text-[11px] sm:text-xs text-slate-400 sm:ml-2 block sm:inline truncate">
                (Sobra após Aportes)
          ...
          <div className="flex items-center space-x-3 sm:space-x-4 shrink-0">
            <span
              className={`text-sm sm:text-xl font-bold text-right ${
